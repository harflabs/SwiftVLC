...
@MainActor
struct PiPControllerInteractionTests {
  @MainActor
  ...
  func `skip backwards from zero clamps to zero`() {
    ...
    #expect(recorder.seekTargets == [0], "Skip clamp should produce target 0")
  }
  ...
  func `skip past duration clamps to duration`() {
    ...
    #expect(recorder.seekTargets == [10000], "Skip target should clamp to duration (10s)")
  }
  ...
  func `skip within bounds passes through unclamped`() {
    ...
    #expect(recorder.seekTargets == [8000])
  }
