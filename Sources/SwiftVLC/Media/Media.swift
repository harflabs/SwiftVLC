...
public final class Media: Sendable {
    ...
    ///
    /// Bridges `libvlc_media_parse_request` → `withCheckedThrowingContinuation`.
    /// - Parameters:
    ...
    public func parse(
        ...
        let result: Result<Metadata, VLCError> = await withCheckedContinuation { cont in
            ...
            let rc = libvlc_media_parse_request(instance.pointer, media, flags, timeoutMs)
            if rc != 0 {
    ...
    /// The media resource locator string.
